...
# ---------- BAZA DANYCH ----------
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get("DATABASE_URL", "sqlite:///magazyn.db")
...
def strona_glowna():
    ...
    return render_template_string(SZABLON, produkty=produkty, kategorie=kategorie, sortuj=sortuj, szukaj=szukaj)
...
def dodaj_produkt():
    ...
    if istnieje:
        ...
        stan_po = ilosc
        produkt_id = nowy.id
    wpis = Historia(produkt_id=produkt_id, produkt_nazwa=nazwa, kategoria_nazwa=kat.nazwa if kat else "—",
                    zmiana=+ilosc, stan_przed=stan_przed, stan_po=stan_po, jednostka=jednostka)
    db.session.add(wpis)
    db.session.commit()
...
def usun_produkt(id):
    ...
    if produkt:
        ...
        wpis = Historia(produkt_id=None, produkt_nazwa=produkt.nazwa, kategoria_nazwa=kat.nazwa if kat else "—",
                        zmiana=-produkt.ilosc, stan_przed=produkt.ilosc, stan_po=0, jednostka=produkt.jednostka)
        db.session.add(wpis)
...
def zmien_ilosc():
    ...
    if produkt:
        ...
        wpis = Historia(produkt_id=produkt.id, produkt_nazwa=produkt.nazwa, kategoria_nazwa=kat.nazwa if kat else "—",
                        zmiana=zmiana, stan_przed=stan_przed, stan_po=stan_po, jednostka=produkt.jednostka)
        db.session.add(wpis)
        db.session.commit()
    return redirect(url_for("strona_glowna"))
    ...
    if wpis and not wpis.cofnieta:
        if wpis.produkt_id:
            ...
            if produkt:
                ...
                cofniecie = Historia(produkt_id=produkt.id, produkt_nazwa=wpis.produkt_nazwa,
                                     kategoria_nazwa=wpis.kategoria_nazwa, zmiana=-wpis.zmiana,
                                     stan_przed=stan_przed_cofnieciem, stan_po=stan_po_cofnieciu, jednostka=wpis.jednostka)
...
def strona_raportu():
    kategorie = Kategoria.query.order_by(Kategoria.nazwa).all()
    return render_template_string(SZABLON_RAPORT, kategorie=kategorie)


@app.route("/raport/generuj")
def generuj_raport():
    data_od = request.args.get("data_od", "")
    data_do = request.args.get("data_do", "")
    kategoria_id = request.args.get("kategoria_id", "")
    query = Historia.query
    if data_od:
        query = query.filter(Historia.data >= datetime.strptime(data_od, "%Y-%m-%d"))
    if data_do:
        data_do_end = datetime.strptime(data_do, "%Y-%m-%d").replace(hour=23, minute=59, second=59)
        query = query.filter(Historia.data <= data_do_end)
    if kategoria_id:
        kat = Kategoria.query.get(int(kategoria_id))
        if kat:
            query = query.filter(Historia.kategoria_nazwa == kat.nazwa)
    wpisy = query.order_by(Historia.data.asc()).all()
    wb = Workbook()
    ws = wb.active
    ws.title = "Raport zmian"
    nf = Font(bold=True, color="FFFFFF", size=11)
    nfill = PatternFill(start_color="1a1a2e", end_color="1a1a2e", fill_type="solid")
    gfill = PatternFill(start_color="eafaf1", end_color="eafaf1", fill_type="solid")
    rfill = PatternFill(start_color="fdedec", end_color="fdedec", fill_type="solid")
    sfill = PatternFill(start_color="d5f5e3", end_color="d5f5e3", fill_type="solid")
    brd = Border(left=Side(style="thin"), right=Side(style="thin"), top=Side(style="thin"), bottom=Side(style="thin"))
    for c, n in enumerate(["Data i godzina", "Produkt", "Kategoria", "Zmiana", "Stan przed", "Stan po", "Cofnięta?"], 1):
        cl = ws.cell(row=1, column=c, value=n); cl.font = nf; cl.fill = nfill
        cl.alignment = Alignment(horizontal="center"); cl.border = brd
    for i, w in enumerate(wpisy, 2):
        ws.cell(row=i, column=1, value=w.data.strftime("%Y-%m-%d %H:%M")).border = brd
        ws.cell(row=i, column=2, value=w.produkt_nazwa).border = brd
        ws.cell(row=i, column=3, value=w.kategoria_nazwa).border = brd
        ws.cell(row=i, column=4, value=w.zmiana).border = brd
        ws.cell(row=i, column=5, value=w.stan_przed).border = brd
        ws.cell(row=i, column=6, value=w.stan_po).border = brd
        ws.cell(row=i, column=7, value="Tak" if w.cofnieta else "Nie").border = brd
        f = gfill if w.zmiana >= 0 else rfill
        for cc in range(1, 8): ws.cell(row=i, column=cc).fill = f
    if wpisy:
        ps = wpisy[0].stan_przed; os_ = wpisy[-1].stan_po; r = len(wpisy) + 3
        ws.cell(row=r, column=1, value="PODSUMOWANIE:").font = Font(bold=True, size=12)
        ws.cell(row=r+1, column=1, value="Stan początkowy:").font = Font(bold=True)
        ws.cell(row=r+1, column=2, value=f"{ps} szt.").font = Font(bold=True)
        ws.cell(row=r+2, column=1, value="Stan końcowy:").font = Font(bold=True)
        ws.cell(row=r+2, column=2, value=f"{os_} szt.").font = Font(bold=True)
        roz = os_ - ps; kr = "27ae60" if roz >= 0 else "e74c3c"
        ws.cell(row=r+3, column=1, value="Różnica:").font = Font(bold=True, size=12)
        cr = ws.cell(row=r+3, column=2, value=f"{roz:+d} szt.")
        cr.font = Font(bold=True, size=12, color=kr)
        for rr in range(r, r+4):
            for cc in range(1, 3): ws.cell(row=rr, column=cc).fill = sfill; ws.cell(row=rr, column=cc).border = brd
    for col, w in zip(["A","B","C","D","E","F","G"], [20,30,25,12,14,14,12]):
        ws.column_dimensions[col].width = w
    out = io.BytesIO(); wb.save(out); out.seek(0)
    return send_file(out, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                     as_attachment=True, download_name=f"raport_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx")
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-raport { background: #27ae60; color: white; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px; }
        .btn-raport:hover { background: #219a52; }
...
<body>
    ...
    <div class="podtytul">
        ...
        <div style="display:flex; gap:10px;">
            <a href="/raport" class="btn-raport">📊 Raport</a>
            <a href="/historia" class="btn-historia">📋 Historia zmian</a>
        </div>
    </div>

    <form class="dodaj-form" action="/dodaj" method="POST">
        <div><label>Nazwa znaku</label><input type="text" name="nazwa" placeholder="np. A-7 (ustap pierwszenstwa)..." required></div>
        <div><label>Ilosc</label><input type="number" name="ilosc" value="1" min="1" required></div>
        <div><label>Jednostka</label><select name="jednostka"><option value="szt.">szt.</option><option value="kpl.">kpl. (komplet)</option><option value="m2">m2 (folia)</option></select></div>
        <div><label>Kategoria</label><select name="kategoria_id" required>{% for kat in kategorie %}<option value="{{ kat.id }}">{{ kat.nazwa }}</option>{% endfor %}</select></div>
        <div><label>Opis (opcjonalnie)</label><input type="text" name="opis" placeholder="np. folia I gen., podklad..."></div>
    ...
    <div class="toolbar">
        ...
        <div class="toolbar-left"><a href="/kategorie" class="btn-outline">⚙️ Kategorie</a></div>
    </div>
    ...
    <div style="margin-bottom: 15px;">
        ...
        <span class="licznik">{% if szukaj %}Znaleziono: {{ produkty|length }}{% else %}Lacznie: {{ produkty|length }}{% endif %} produkt(ow)</span>
    </div>

    {% if produkty %}
    <div class="tabela-wrapper"><table><thead><tr><th>Znak</th><th>Kategoria</th><th>Ilosc</th><th>Opis</th><th>Akcje</th></tr></thead><tbody>
    {% for p in produkty %}<tr>
        <td><strong>{{ p.nazwa }}</strong></td>
        <td>{% set kat_klasa = 'kat-inna' %}{% if 'A (' in p.kategoria.nazwa or p.kategoria.nazwa.startswith('Znaki A') %}{% set kat_klasa = 'kat-A' %}{% elif 'B (' in p.kategoria.nazwa or p.kategoria.nazwa.startswith('Znaki B') %}{% set kat_klasa = 'kat-B' %}{% elif 'D (' in p.kategoria.nazwa or p.kategoria.nazwa.startswith('Znaki D') %}{% set kat_klasa = 'kat-D' %}{% endif %}<span class="kat-badge {{ kat_klasa }}">{{ p.kategoria.nazwa }}</span></td>
        <td><span class="{% if p.ilosc <= 5 %}ilosc-niska{% else %}ilosc-ok{% endif %}">{{ p.ilosc }} {{ p.jednostka }}</span></td>
        <td>{{ p.opis }}</td>
        <td><div class="przyciski">
    ...
    {% else %}<p class="pusto">{% if szukaj %}🔍 Nic nie znaleziono dla "{{ szukaj }}"{% else %}📭 Magazyn jest pusty. Dodaj pierwszy znak powyzej!{% endif %}</p>{% endif %}

    <div class="modal" id="modalKrok1"><div class="modal-content"><h3 id="k1Tytul">Zmiana ilosci</h3><div class="produkt-info" id="k1Info"></div><input type="number" id="k1Wartosc" value="1" min="1" required><div class="modal-buttons"><button class="btn-anuluj" onclick="zamknijWszystko()">Anuluj</button><button class="btn-potwierdz" id="k1Dalej">Dalej →</button></div></div></div>
    <div class="modal" id="modalKrok2"><div class="modal-content"><h3 id="k2Tytul">Potwierdz zmiane</h3><div class="produkt-info" id="k2Info"></div><div class="podsumowanie"><span>Stan przed:</span><span id="k2Przed">0</span></div><div class="podsumowanie"><span>Zmiana:</span><span id="k2Zmiana">0</span></div><div class="podsumowanie" style="border-top: 1px solid #eee; padding-top: 8px; margin-top: 5px;"><span>Stan po:</span><span style="font-size: 18px; font-weight: bold;" id="k2Po">0</span></div><div class="modal-buttons" style="margin-top: 20px;"><button class="btn-anuluj" onclick="cofnijDoKroku1()">← Wstecz</button><button class="btn-anuluj" onclick="zamknijWszystko()">Anuluj</button><button class="btn-potwierdz" id="k2Zatwierdz">✅ Zatwierdz</button></div></div></div>
    <form id="zmianaForm" action="/zmien" method="POST" style="display:none;"><input type="hidden" name="id" id="zmianaId"><input type="hidden" name="akcja" id="zmianaAkcja"><input type="hidden" name="wartosc" id="zmianaWartosc"></form>
    ...
    <script>
        ...
        function otworzModalKrok1(id, akcja, nazwa, ilosc, jednostka, kategoria) {
            ...
            document.getElementById('k1Info').innerHTML = '<strong>' + nazwa + '</strong><br>Kategoria: ' + kategoria + '<br>Obecny stan: <strong>' + ilosc + ' ' + jednostka + '</strong>';
            document.getElementById('k1Wartosc').value = 1;
            if (akcja === 'plus') { document.getElementById('k1Tytul').textContent = '📥 Przyjecie na magazyn'; document.getElementById('k1Dalej').className = 'btn-potwierdz'; }
            else { document.getElementById('k1Tytul').textContent = '📤 Wydanie z magazynu'; document.getElementById('k1Dalej').className = 'btn-potwierdz odejmij'; }
            document.getElementById('modalKrok1').classList.add('aktywny'); document.getElementById('k1Wartosc').focus(); document.getElementById('k1Wartosc').select();
        }
        function przejdzDoKroku2() {
            var w = parseInt(document.getElementById('k1Wartosc').value);
            if (isNaN(w) || w < 1) { alert('Wpisz poprawna ilosc (minimum 1)!'); return; }
            daneZmiany.wartosc = w;
            var sp = daneZmiany.ilosc, spo = (daneZmiany.akcja === 'plus') ? sp + w : Math.max(0, sp - w);
            document.getElementById('k2Info').innerHTML = '<strong>' + daneZmiany.nazwa + '</strong> | ' + daneZmiany.kategoria;
            document.getElementById('k2Przed').textContent = sp + ' ' + daneZmiany.jednostka;
            document.getElementById('k2Po').textContent = spo + ' ' + daneZmiany.jednostka;
            var zm = (daneZmiany.akcja === 'plus') ? w : -w;
            document.getElementById('k2Zmiana').innerHTML = zm > 0 ? '<span class="zmiana-plus">+' + zm + ' ' + daneZmiany.jednostka + '</span>' : '<span class="zmiana-minus">' + zm + ' ' + daneZmiany.jednostka + '</span>';
            document.getElementById('k2Tytul').textContent = daneZmiany.akcja === 'plus' ? '📥 Potwierdz przyjecie' : '📤 Potwierdz wydanie';
            document.getElementById('k2Zatwierdz').className = daneZmiany.akcja === 'plus' ? 'btn-potwierdz' : 'btn-potwierdz odejmij';
            document.getElementById('modalKrok1').classList.remove('aktywny'); document.getElementById('modalKrok2').classList.add('aktywny');
        }
        function cofnijDoKroku1() { document.getElementById('modalKrok2').classList.remove('aktywny'); document.getElementById('modalKrok1').classList.add('aktywny'); document.getElementById('k1Wartosc').focus(); }
        function zamknijWszystko() { document.getElementById('modalKrok1').classList.remove('aktywny'); document.getElementById('modalKrok2').classList.remove('aktywny'); }
        function zatwierdzZmiane() { document.getElementById('zmianaId').value = daneZmiany.id; document.getElementById('zmianaAkcja').value = daneZmiany.akcja; document.getElementById('zmianaWartosc').value = daneZmiany.wartosc; document.getElementById('zmianaForm').submit(); }
        ...
        document.getElementById('szukajInput').addEventListener('input', function() {
            ...
            var v = this.value;
            szukajTimeout = setTimeout(function() { var url = new URL(window.location); if (v) url.searchParams.set('szukaj', v); else url.searchParams.delete('szukaj'); window.location = url.toString(); }, 300);
        });
        document.addEventListener('keydown', function(e) { if ((e.ctrlKey || e.metaKey) && e.key === 'f') { e.preventDefault(); document.getElementById('szukajInput').focus(); } });
    </script>
</body>
</html>
"""
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) { .wpis { flex-direction: column; align-items: flex-start; } .wpis-prawy { text-align: left; } }
    </style>
</head>
<body>
    <h1>📋 Historia zmian</h1><p class="podtytul">Wszystkie operacje magazynowe — od najnowszych</p>
    <a href="/" class="btn-powrot">← Powrot do magazynu</a>
    {% if wpisy %}
        {% set ns = namespace(data='') %}
        {% for w in wpisy %}
            {% set dd = w.data.strftime('%Y-%m-%d') %}
            {% if dd != ns.data %}{% set ns.data = dd %}<div class="data-naglowek">📅 {{ w.data.strftime('%d.%m.%Y') }}</div>{% endif %}
            ...
            <div class="wpis {{ klasa }} {% if w.cofnieta %}cofniety{% endif %}">
                <div class="wpis-lewy">
                    <strong>{{ w.produkt_nazwa }}</strong>{% if w.cofnieta %}<span style="color:#e74c3c;font-size:11px;"> (COFNIETA)</span>{% endif %}
                    <div class="kat">{{ w.kategoria_nazwa }}</div><div class="meta">{{ w.data.strftime('%H:%M:%S') }}</div>
                </div>
                <div class="wpis-srodek"><div class="zmiana-ilosc">{% if w.zmiana > 0 %}<span class="zmiana-plus">+{{ w.zmiana }} {{ w.jednostka }}</span>{% else %}<span class="zmiana-minus">{{ w.zmiana }} {{ w.jednostka }}</span>{% endif %}</div></div>
                <div class="wpis-prawy">
                    <div class="stan">{{ w.stan_przed }} → <span>{{ w.stan_po }}</span> {{ w.jednostka }}</div>
                    {% if not w.cofnieta and w.produkt_id %}<a href="/cofnij/{{ w.id }}" class="btn-cofnij" onclick="return confirm('Cofnac te zmiane?')">↩ Cofnij</a>{% endif %}
                </div>
            </div>
        {% endfor %}
    {% else %}<p class="pusto">📭 Brak wpisow w historii.</p>{% endif %}
</body>
</html>
"""
...
<body>
    ...
    {% if kategorie %}
    <ul>{% for kat in kategorie %}<li><span><strong>{{ kat.nazwa }}</strong> <span class="info">({{ kat.produkty|length }} produktow)</span></span><a href="/usun_kategorie/{{ kat.id }}" class="btn-usun" onclick="return confirm('Usuniecie kategorii usunie tez wszystkie znaki w niej! Kontynuowac?')">🗑 Usun</a></li>{% endfor %}</ul>
    {% else %}<p style="color:#999; text-align:center; padding:30px;">Brak kategorii. Dodaj pierwsza!</p>{% endif %}
    <a href="/" class="btn-powrot">← Powrot do magazynu</a>
</body>
</html>
"""
...
SZABLON_RAPORT = r"""
<!DOCTYPE html><html lang="pl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Raport - Magazyn Znakow</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;max-width:700px;margin:30px auto;padding:0 15px;background:#f0f2f5}h1{color:#1a1a2e;margin-bottom:5px}.podtytul{color:#666;margin-bottom:25px;font-size:14px}.btn-powrot{display:inline-block;margin-bottom:20px;color:#4a90d9;text-decoration:none;font-weight:bold;font-size:14px}.karta{background:white;padding:30px;border-radius:12px;box-shadow:0 2px 15px rgba(0,0,0,0.08)}.fg{margin-bottom:20px}.fg label{display:block;font-weight:bold;margin-bottom:6px;color:#333;font-size:14px}.fg input,.fg select{width:100%;padding:12px;border:2px solid #ddd;border-radius:8px;font-size:14px}.fg input:focus,.fg select:focus{outline:none;border-color:#4a90d9}.fr{display:flex;gap:15px}.fr .fg{flex:1}.btn-gen{width:100%;padding:14px;background:#27ae60;color:white;border:none;border-radius:8px;font-size:16px;font-weight:bold;cursor:pointer;margin-top:10px}.btn-gen:hover{background:#219a52}.info{background:#eaf2f8;padding:15px;border-radius:8px;margin-top:20px;font-size:13px;color:#2c3e50;line-height:1.6}@media(max-width:600px){.fr{flex-direction:column;gap:0}}</style></head>
<body><h1>📊 Raport zmian</h1><p class="podtytul">Wygeneruj raport Excel dla wybranego okresu</p><a href="/" class="btn-powrot">← Powrot do magazynu</a>
<div class="karta"><form action="/raport/generuj" method="GET">
<div class="fr"><div class="fg"><label>📅 Data od:</label><input type="date" name="data_od"></div><div class="fg"><label>📅 Data do:</label><input type="date" name="data_do"></div></div>
<div class="fg"><label>📂 Kategoria (opcjonalnie):</label><select name="kategoria_id"><option value="">-- Wszystkie kategorie --</option>{% for kat in kategorie %}<option value="{{ kat.id }}">{{ kat.nazwa }}</option>{% endfor %}</select></div>
<button type="submit" class="btn-gen">📥 Generuj raport Excel</button></form>
<div class="info">💡 <strong>Jak to dziala:</strong><br>• Raport zawiera wszystkie zmiany z wybranego okresu<br>• Mozesz zawezic do jednej kategorii<br>• Podsumowanie pokazuje stan poczatkowy, koncowy i roznice<br>• Jesli nie wybierzesz dat — raport obejmie cala historie</div></div>
<script>var d=new Date().toISOString().split('T')[0];document.querySelector('[name=data_od]').value=d;document.querySelector('[name=data_do]').value=d</script></body></html>"""
